        ...
        */
        ...
        public void Task4()
        {
            var res = Emps.Where(emp =>
                emp.Salary == Emps.Max(emp => emp.Salary)
                ).ToList();
        ...
        public void Task6()
        {
            var res = Emps.Join(Depts,
                empDepno => empDepno.Deptno,
                deptno => deptno.Deptno,
                (empDepno, deptno) => new {Emps = empDepno.Ename, empDepno.Job, Depts = deptno.Dname}).ToList();
        }
        ...
        public void Task7()
        {
            var res = Emps.GroupBy(emp => emp.Job).Select(emp => new
            {
                EmployeeJob = emp.Key,
                count = emp.Count()
            }).ToList();
        ...
        {
            var res = Emps
                .Select(emp => (emp.Ename, emp.Job, emp.HireDate))
                .Union(Emps
                    .Select(emp2 => (Ename: "No value", Job: (string)null, HireDate: (DateTime?)null)));
        }

        //Find the employee with the highest salary using the Aggregate () method
        public void Task11()
        {
            var res = Emps.Aggregate(0, (highest, emp) => emp.Salary > highest ? emp.Salary : highest).ToString();
        ...
        public void Task12()
        {
            var res = Emps.SelectMany(emp => Depts, (empR, deptR) => new
            { empR.Ename, empR.Job, empR.Salary, empR.HireDate, deptR.Deptno, deptR.Dname, deptR.Loc }).ToList();
